    ...
    private static void DecodeRgb565_Sse2(ReadOnlySpan<byte> src, byte* dst, int w, int h, bool littleEndian, bool swapRgbChannels)
    {
        fixed (byte* pSrc = src)
        {
            for (int y = 0; y < h; y++)
            {
                ...
                if (swapRgbChannels)
                {
                    // BGR565 layout: BBBBB GGGGGG RRRRR
                    for (; x + 7 < w; x += 8)
                    {
                        ...
                        // BGR565: B bits 15-11, G bits 10-5, R bits 4-0
                        var b5 = Sse2.And(Sse2.ShiftRightLogical(v, 11), Vector128.Create((ushort)0x001F));
                ...
                DecodeRgb565_Tail(pSrcRow, pDstRow, x, w, littleEndian, swapRgbChannels);
            }
        }
    }
            ...
            for (int y = 0; y < h; y++)
            {
                ...
                if (swapRgbChannels)
                {
                    ...
                    for (; x + 7 < w; x += 8)
                    {
                        ...
                        // BGR565: B bits 15-11, G bits 10-5, R bits 4-0
                        var b5 = AdvSimd.And(AdvSimd.ShiftRightLogical(v, 11), Vector128.Create((ushort)0x001F));
